...
class AllScoresCalculator {
  ...
  Future<ComprehensiveReadinessResult> calculateAll({
    ...
    final Map<String, Map<String, dynamic>> componentBreakdown = {
      'Recovery': recovery.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Sleep Index': sleep.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Fatigue Index': fatigue.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Endurance': endurance.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Cardio Fitness': cardioFitness.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Stress Load': stressLoad.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Injury Risk': injuryRisk.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Cardio-Resp Stability': cardioResp.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Illness Risk': illnessRisk.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Daily Activity': dailyActivity.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Work Capacity': workCapacity.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Altitude': altitude.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Cardiac Safety': cardiacSafety.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Sleep Debt': sleepDebt.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Training Readiness': trainingReadiness.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Cognitive Alertness': cognitiveAlertness.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
      'Thermoregulatory': thermoreg.components.map((k, v) => MapEntry(k, (v as num).toDouble())),
    };
